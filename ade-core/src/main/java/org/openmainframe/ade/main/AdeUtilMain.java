...
public class AdeUtilMain extends ControlProgram {
	...
	protected void dumpModelDebug(File modelFile, File outputFile)
			throws AdeException, AdeUsageException,
			AdeInternalException {
		IMainScorer model = Ade.getAde().getDataStore().models().readModelFromFile(modelFile);
		PrintStream outStream;
		if (outputFile==null){
			outputFile = new File(Ade.getAde().getDirectoryManager().getOutputHome(), "model_debug.txt");
		}
		try {
			System.out.println("Writing model debug information to "+outputFile.getAbsolutePath());
			outStream = new PrintStream(outputFile);
		} catch (IOException e) {
			throw new AdeUsageException("Failed writing debug information to file", e);
		}

		try {
			model.debugPrint(outStream);
		} catch (Exception e) {
			throw new AdeInternalException("Failed generating debug information of model", e);
		}
		outStream.close();
	}
