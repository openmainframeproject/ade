 ...
 * appropriate to write a final statistic, the eof() should be called,
 * finalizing the cumulative time measure, so that the last (partial) stat can
 * be realized and appended.
 *
 * NOTES: We rely on the System class' ability to accurately portray
 * nanosecond-based relative timings in order to determine relative time
 * measures. All such timings are *purely* relative ones.
 *
 */
public abstract class PeriodicStats {
	 ...
	 *
	 * @param who
	 *            String which identifies the caller in some way
	 * @param headerLines
	 *            String[] array which should describe the remaining content of
	 *            the stats file
	 * @throws AdeInternalException
	 */
	protected final void initStatistics(String who, String[] headerLines)
			throws AdeException {

		setStatsFilePath();
		if (statsFilePath == null) {
			throw new AdeInternalException(
					"No Statistics File path was specified");
		}

		appendStatString(SPACER);
		appendStatString(String.format("[%s] %s start (approximate)",
				currentTimeStamp(), who));
		appendStatString(SPACER);
	...
	protected final void eof(String who) throws AdeInternalException {
		appendStatString(SPACER);
		appendStatString(String.format("[%s] %s end (approximate)",
				currentTimeStamp(), who));
		appendStatString(SPACER);
	}
	...
	/**
	 * Method to append to the stats file corresponding to this instance, and
	 * start a new timing start value
	 */
	...
	/**
	 * Convenience overload for rounding to the default (2) number of decimal
	 * places
	 *
	 * @param numToRound
	 * @return double
	...
	/**
	 * self-explanatory method for generating a printable timestamp according to
	 * the standard date format string
	 *
	...
	private void appendStatString(String textLine) throws AdeInternalException {
		...
		try {
			fos = new FileOutputStream(statsFilePath, true);
	...
	/**
	 * self-explanatory method for converting a nanos measurement into a
	 * best-fit equivalent seconds measurement
	 *
