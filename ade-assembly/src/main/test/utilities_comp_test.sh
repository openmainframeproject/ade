...
#************************************************************************
# Script: utilities_comp_test.sh
#
# Usage: utilities_comp_test.sh
#
...
BASELINE_UPLOAD_DIR="$BASELINE_DIR/syslog/upload"
BASELINE_ANALYZE_DIR="$BASELINE_DIR/syslog/analyze"
...
# Build the location of the model file (.bin) file created
# Use test output directory value
# Append models/event_log_model_1.bin 
find_model_file() {

  echo "test output directory value: $test_outdir_name"
  MODEL_INPUT_FILE="$test_outdir_name/models/event_log_model_1.bin"
  MODEL_OUTPUT_FILE="$test_outdir_name/models/event_log_model_1.txt"
...
check_test_env() {
  ...
  if [ ! -d "$BASELINE_UPLOAD_DIR" ]; then
    echo "ERROR: Unable to locate baseline test syslog upload directory: $BASELINE_UPLOAD_DIR"
    exit 1
  fi

  if [ ! -d "$BASELINE_ANALYZE_DIR" ]; then
    echo "ERROR: Unable to locate baseline test syslog analyze directory: $BASELINE_ANALYZE_DIR"
    exit 1
  fi
...
# Find path to model (.bin) file
find_model_file
if [ $? -ne 0 ]; then
  echo "Failed to retrieve location of model file. Exiting"
  cleanup_and_exit 1
fi
...
issue_command "$BIN_DIR/train $AG_NAME 11/22/2015 11/30/2015"
if [ $? -ne 0 ]; then
...
## verify  that flowlayout.xml 

echo
echo "Verifying FlowLayout.xml ..."

issue_command "$BIN_DIR/utilities -f $XML_DIR/FlowLayout.xml "
if [ $? -ne 0 ]; then
  echo "ERROR: FlowLayout.xml does not match FlowLayout.xsd $MODEL_OUTPUT_DIR. Exiting"
  cleanup_and_exit 1
fi 
echo "FlowLayout.xml verified "

## create text summary of model

echo
echo "Checking content of model file ..."
issue_command "$BIN_DIR/utilities -d $MODEL_INPUT_FILE -o $MODEL_OUTPUT_FILE"
if [ $? -ne 0 ]; then
  echo "ERROR: Failed to print text summary of model from $MODEL_OUTPUT_DIR. Exiting"
  cleanup_and_exit 1
fi 
